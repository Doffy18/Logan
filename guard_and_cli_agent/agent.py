...
from ingestion.database import init_sqlite_db, init_chroma_db
...
def retrieve_context_from_dbs(query: str, db_path: str = "codebase_metadata.db") -> str:
    """Helper function to fetch vector chunks from ChromaDB and symbols from SQLite."""
    ...
    try:
        ...
        if results and results.get("documents") and results["documents"][0]:
            ...
            for doc, meta in zip(results["documents"][0], results["metadatas"][0]):
                context_blocks.append(f"File: {meta.get('file_path')}\n```python\n{doc}\n```")
    ...
    try:
        ...
        if keywords:
            cursor.execute(
                "SELECT file_path, symbol_type, name, start_line, end_line FROM symbols WHERE name LIKE ? LIMIT 5",
                (keywords[0],)
            )
...
def generator_node(state: AgentState) -> Dict[str, Any]:
    """Node 3: Generates streamed response using LangChain init_chat_model."""
    console.print("[dim]⚡ Node: generator_node (Invoking Gemini via init_chat_model)...[/dim]")

    model = init_chat_model(model = 'gemini-2.5-flash', model_provider='google_genai', api_key=api_key)
    ...
    print("\n")
    return {"messages": [HumanMessage(content=state.query), model.invoke([system_message, user_msg])]}
